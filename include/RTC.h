...
public:
    void RTC_init(void);
    void get_time(void);
    String format_date(void);
    String format_time(void);
    void show_time(void);
    void show_time_json(void);
};
...
void DS1307_RTC::RTC_init(void)
{
    ...
    if (!rtc.isrunning())
    { /* Si el reloj no está ejecutándose*/
    ...
    // rtc.adjust(DateTime(2014, 1, 21, 3, 0, 0));  // año, mes, día, hora, minuto, segundo.                  /* Función que ajusta manualmente la fecha y hora. */
}
...
void DS1307_RTC ::show_time_json(void)
{
}
